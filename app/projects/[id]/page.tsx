    ...
    <div className="min-h-screen bg-gradient-to-br from-slate-50 to-slate-100 dark:from-slate-900 dark:to-slate-800">
      ...
      <header className="border-b border-slate-200 dark:border-slate-700 bg-white/50 dark:bg-slate-900/50 backdrop-blur-sm sticky top-0 z-10">
        <nav className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-4">
          <Link href="/" className="text-blue-600 hover:text-blue-700 dark:text-blue-400 dark:hover:text-blue-300 transition">
            ← Retour au portfolio
          </Link>
        </nav>
      </header>
      ...
      <main className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-12">
        ...
        {/* 3D Model */}
        <div className="bg-white dark:bg-slate-800 rounded-xl shadow-lg p-6 mb-8" style={{ height: '400px' }}>
          <ThreeModel modelType={project.modelType} scaleMultiplier={id === '1' ? 1.8 : 1} />
        </div>
        ...
        <div className="bg-white dark:bg-slate-800 rounded-xl shadow-lg p-8 mb-8">
          ...
          <div className="grid grid-cols-2 md:grid-cols-4 gap-4">
            {project.technologies.map((tech, index) => (
              <div
                key={index}
                className="bg-slate-100 dark:bg-slate-700 rounded-lg p-4 text-center font-medium text-slate-900 dark:text-white"
              >
                {tech}
              </div>
            ))}
        ...
        <div className="bg-white dark:bg-slate-800 rounded-xl shadow-lg p-8">
          ...
          <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
            {project.images.map((image, index) => (
              <div key={index} className="rounded-lg overflow-hidden bg-slate-200 dark:bg-slate-700">
                <img
                  src={image}
